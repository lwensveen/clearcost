...
import { deMinimisBasis, deMinimisKind, deMinimisSourceEnum } from '../enums.js';
...
export const deMinimisTable = pgTable(
  'de_minimis',
  {
    ...
    deMinimisBasis: text('de_minimis_basis', { enum: deMinimisBasis })
      .notNull()
      .default('INTRINSIC'),
    source: deMinimisSourceEnum('source').notNull().default('official'),
  ...
  (t) => [
    ...
    index('idx_dem_source').on(t.source),
  ]
);
