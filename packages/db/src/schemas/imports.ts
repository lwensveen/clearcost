...
export const importsTable = pgTable(
  ...
  (t) => ({
    idxJobStatus: index('idx_imports_job_status').on(t.job, t.importStatus),
    idxSourceKeyStatus: index('idx_imports_source_key_status').on(t.sourceKey, t.importStatus),
    idxFinishedAt: index('idx_imports_finished_at').on(t.finishedAt),
  })
);
