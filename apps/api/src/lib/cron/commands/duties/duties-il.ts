...
import { importIlMfnOfficial } from '../../../../modules/duty-rates/services/il/import-mfn-official.js';
import {
  IL_FTA_OFFICIAL_SOURCE_KEY,
  IL_MFN_OFFICIAL_SOURCE_KEY,
  resolveIlDutySourceUrls,
} from '../../../../modules/duty-rates/services/il/source-urls.js';
...
async function runIlMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveIlDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'IL MFN source URL is required. Provide --url=<source> or configure duties.il.official.mfn_excel / IL_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importIlMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'il-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runIlFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveIlDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'IL FTA source URL is required. Provide --url=<source> or configure duties.il.official.fta_excel / IL_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'il-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesIlMfnOfficial: Command = async (args) => {
  const totals = await runIlMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesIlFtaOfficial: Command = async (args) => {
  const totals = await runIlFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesIlAllOfficial: Command = async (args) => {
  const mfn = await runIlMfnOfficial(args);
  const fta = await runIlFtaOfficial(args);
  console.log({
    ok: true,
