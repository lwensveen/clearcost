...
import { importCzMfnOfficial } from '../../../../modules/duty-rates/services/cz/import-mfn-official.js';
import {
  CZ_FTA_OFFICIAL_SOURCE_KEY,
  CZ_MFN_OFFICIAL_SOURCE_KEY,
  resolveCzDutySourceUrls,
} from '../../../../modules/duty-rates/services/cz/source-urls.js';
...
async function runCzMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveCzDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'CZ MFN source URL is required. Provide --url=<source> or configure duties.cz.official.mfn_excel / CZ_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importCzMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'cz-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runCzFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveCzDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'CZ FTA source URL is required. Provide --url=<source> or configure duties.cz.official.fta_excel / CZ_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'cz-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesCzMfnOfficial: Command = async (args) => {
  const totals = await runCzMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesCzFtaOfficial: Command = async (args) => {
  const totals = await runCzFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesCzAllOfficial: Command = async (args) => {
  const mfn = await runCzMfnOfficial(args);
  const fta = await runCzFtaOfficial(args);
  console.log({
    ok: true,
