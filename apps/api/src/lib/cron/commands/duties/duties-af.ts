...
import { importAfMfnOfficial } from '../../../../modules/duty-rates/services/af/import-mfn-official.js';
import {
  AF_FTA_OFFICIAL_SOURCE_KEY,
  AF_MFN_OFFICIAL_SOURCE_KEY,
  resolveAfDutySourceUrls,
} from '../../../../modules/duty-rates/services/af/source-urls.js';
...
async function runAfMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveAfDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'AF MFN source URL is required. Provide --url=<source> or configure duties.af.official.mfn_excel / AF_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importAfMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'af-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runAfFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveAfDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'AF FTA source URL is required. Provide --url=<source> or configure duties.af.official.fta_excel / AF_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'af-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesAfMfnOfficial: Command = async (args) => {
  const totals = await runAfMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesAfFtaOfficial: Command = async (args) => {
  const totals = await runAfFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesAfAllOfficial: Command = async (args) => {
  const mfn = await runAfMfnOfficial(args);
  const fta = await runAfFtaOfficial(args);
  console.log({
    ok: true,
