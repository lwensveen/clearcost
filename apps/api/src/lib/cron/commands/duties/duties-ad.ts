...
import { importAdMfnOfficial } from '../../../../modules/duty-rates/services/ad/import-mfn-official.js';
import {
  AD_FTA_OFFICIAL_SOURCE_KEY,
  AD_MFN_OFFICIAL_SOURCE_KEY,
  resolveAdDutySourceUrls,
} from '../../../../modules/duty-rates/services/ad/source-urls.js';
...
async function runAdMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveAdDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'AD MFN source URL is required. Provide --url=<source> or configure duties.ad.official.mfn_excel / AD_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importAdMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'ad-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runAdFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveAdDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'AD FTA source URL is required. Provide --url=<source> or configure duties.ad.official.fta_excel / AD_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'ad-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesAdMfnOfficial: Command = async (args) => {
  const totals = await runAdMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesAdFtaOfficial: Command = async (args) => {
  const totals = await runAdFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesAdAllOfficial: Command = async (args) => {
  const mfn = await runAdMfnOfficial(args);
  const fta = await runAdFtaOfficial(args);
  console.log({
    ok: true,
