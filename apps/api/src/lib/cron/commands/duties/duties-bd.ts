...
import { importBdMfnOfficial } from '../../../../modules/duty-rates/services/bd/import-mfn-official.js';
import {
  BD_FTA_OFFICIAL_SOURCE_KEY,
  BD_MFN_OFFICIAL_SOURCE_KEY,
  resolveBdDutySourceUrls,
} from '../../../../modules/duty-rates/services/bd/source-urls.js';
...
async function runBdMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveBdDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'BD MFN source URL is required. Provide --url=<source> or configure duties.bd.official.mfn_excel / BD_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importBdMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'bd-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runBdFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveBdDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'BD FTA source URL is required. Provide --url=<source> or configure duties.bd.official.fta_excel / BD_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'bd-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesBdMfnOfficial: Command = async (args) => {
  const totals = await runBdMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesBdFtaOfficial: Command = async (args) => {
  const totals = await runBdFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesBdAllOfficial: Command = async (args) => {
  const mfn = await runBdMfnOfficial(args);
  const fta = await runBdFtaOfficial(args);
  console.log({
    ok: true,
