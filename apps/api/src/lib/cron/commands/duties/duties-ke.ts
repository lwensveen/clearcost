...
import { importKeMfnOfficial } from '../../../../modules/duty-rates/services/ke/import-mfn-official.js';
import {
  KE_FTA_OFFICIAL_SOURCE_KEY,
  KE_MFN_OFFICIAL_SOURCE_KEY,
  resolveKeDutySourceUrls,
} from '../../../../modules/duty-rates/services/ke/source-urls.js';
...
async function runKeMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveKeDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'KE MFN source URL is required. Provide --url=<source> or configure duties.ke.official.mfn_excel / KE_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importKeMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'ke-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runKeFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveKeDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'KE FTA source URL is required. Provide --url=<source> or configure duties.ke.official.fta_excel / KE_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'ke-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesKeMfnOfficial: Command = async (args) => {
  const totals = await runKeMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesKeFtaOfficial: Command = async (args) => {
  const totals = await runKeFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesKeAllOfficial: Command = async (args) => {
  const mfn = await runKeMfnOfficial(args);
  const fta = await runKeFtaOfficial(args);
  console.log({
    ok: true,
