...
import { importAeMfnOfficial } from '../../../../modules/duty-rates/services/ae/import-mfn-official.js';
import {
  AE_FTA_OFFICIAL_SOURCE_KEY,
  AE_MFN_OFFICIAL_SOURCE_KEY,
  resolveAeDutySourceUrls,
} from '../../../../modules/duty-rates/services/ae/source-urls.js';
...
async function runAeMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveAeDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'AE MFN source URL is required. Provide --url=<source> or configure duties.ae.official.mfn_excel / AE_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importAeMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'ae-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runAeFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveAeDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'AE FTA source URL is required. Provide --url=<source> or configure duties.ae.official.fta_excel / AE_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'ae-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesAeMfnOfficial: Command = async (args) => {
  const totals = await runAeMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesAeFtaOfficial: Command = async (args) => {
  const totals = await runAeFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesAeAllOfficial: Command = async (args) => {
  const mfn = await runAeMfnOfficial(args);
  const fta = await runAeFtaOfficial(args);
  console.log({
    ok: true,
