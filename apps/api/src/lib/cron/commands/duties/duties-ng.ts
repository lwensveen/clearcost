...
import { importNgMfnOfficial } from '../../../../modules/duty-rates/services/ng/import-mfn-official.js';
import {
  NG_FTA_OFFICIAL_SOURCE_KEY,
  NG_MFN_OFFICIAL_SOURCE_KEY,
  resolveNgDutySourceUrls,
} from '../../../../modules/duty-rates/services/ng/source-urls.js';
...
async function runNgMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveNgDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'NG MFN source URL is required. Provide --url=<source> or configure duties.ng.official.mfn_excel / NG_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importNgMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'ng-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runNgFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveNgDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'NG FTA source URL is required. Provide --url=<source> or configure duties.ng.official.fta_excel / NG_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'ng-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesNgMfnOfficial: Command = async (args) => {
  const totals = await runNgMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesNgFtaOfficial: Command = async (args) => {
  const totals = await runNgFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesNgAllOfficial: Command = async (args) => {
  const mfn = await runNgMfnOfficial(args);
  const fta = await runNgFtaOfficial(args);
  console.log({
    ok: true,
