...
import { importHuMfnOfficial } from '../../../../modules/duty-rates/services/hu/import-mfn-official.js';
import {
  HU_FTA_OFFICIAL_SOURCE_KEY,
  HU_MFN_OFFICIAL_SOURCE_KEY,
  resolveHuDutySourceUrls,
} from '../../../../modules/duty-rates/services/hu/source-urls.js';
...
async function runHuMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveHuDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'HU MFN source URL is required. Provide --url=<source> or configure duties.hu.official.mfn_excel / HU_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importHuMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'hu-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runHuFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveHuDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'HU FTA source URL is required. Provide --url=<source> or configure duties.hu.official.fta_excel / HU_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'hu-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesHuMfnOfficial: Command = async (args) => {
  const totals = await runHuMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesHuFtaOfficial: Command = async (args) => {
  const totals = await runHuFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesHuAllOfficial: Command = async (args) => {
  const mfn = await runHuMfnOfficial(args);
  const fta = await runHuFtaOfficial(args);
  console.log({
    ok: true,
