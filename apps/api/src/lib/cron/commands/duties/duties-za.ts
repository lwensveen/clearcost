...
import { importZaMfnOfficial } from '../../../../modules/duty-rates/services/za/import-mfn-official.js';
import {
  ZA_FTA_OFFICIAL_SOURCE_KEY,
  ZA_MFN_OFFICIAL_SOURCE_KEY,
  resolveZaDutySourceUrls,
} from '../../../../modules/duty-rates/services/za/source-urls.js';
...
async function runZaMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveZaDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'ZA MFN source URL is required. Provide --url=<source> or configure duties.za.official.mfn_excel / ZA_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importZaMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'za-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runZaFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveZaDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'ZA FTA source URL is required. Provide --url=<source> or configure duties.za.official.fta_excel / ZA_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'za-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesZaMfnOfficial: Command = async (args) => {
  const totals = await runZaMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesZaFtaOfficial: Command = async (args) => {
  const totals = await runZaFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesZaAllOfficial: Command = async (args) => {
  const mfn = await runZaMfnOfficial(args);
  const fta = await runZaFtaOfficial(args);
  console.log({
    ok: true,
