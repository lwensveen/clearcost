...
import { importSaMfnOfficial } from '../../../../modules/duty-rates/services/sa/import-mfn-official.js';
import {
  SA_FTA_OFFICIAL_SOURCE_KEY,
  SA_MFN_OFFICIAL_SOURCE_KEY,
  resolveSaDutySourceUrls,
} from '../../../../modules/duty-rates/services/sa/source-urls.js';
...
async function runSaMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveSaDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'SA MFN source URL is required. Provide --url=<source> or configure duties.sa.official.mfn_excel / SA_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importSaMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'sa-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runSaFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveSaDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'SA FTA source URL is required. Provide --url=<source> or configure duties.sa.official.fta_excel / SA_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'sa-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesSaMfnOfficial: Command = async (args) => {
  const totals = await runSaMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesSaFtaOfficial: Command = async (args) => {
  const totals = await runSaFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesSaAllOfficial: Command = async (args) => {
  const mfn = await runSaMfnOfficial(args);
  const fta = await runSaFtaOfficial(args);
  console.log({
    ok: true,
