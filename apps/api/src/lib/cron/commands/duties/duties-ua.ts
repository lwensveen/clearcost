...
import { importUaMfnOfficial } from '../../../../modules/duty-rates/services/ua/import-mfn-official.js';
import {
  UA_FTA_OFFICIAL_SOURCE_KEY,
  UA_MFN_OFFICIAL_SOURCE_KEY,
  resolveUaDutySourceUrls,
} from '../../../../modules/duty-rates/services/ua/source-urls.js';
...
async function runUaMfnOfficial(args: string[]): Promise<ImportTotals> {
  ...
  const { mfnUrl } = await resolveUaDutySourceUrls({ mfnUrl: mfnUrlOverride });

  if (!mfnUrl) {
    throw new Error(
      'UA MFN source URL is required. Provide --url=<source> or configure duties.ua.official.mfn_excel / UA_MFN_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  const step = await withRun(
    ...
    async (importId) => {
      const result = await importUaMfnOfficial({
        urlOrPath: mfnUrl,
        sheet,
        batchSize,
        dryRun,
  ...
  console.log({ step: 'ua-mfn-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

async function runUaFtaOfficial(args: string[]): Promise<ImportTotals> {
  const flags = parseFlags(args);
  const batchSize = flagNum(flags, 'batchSize');
  const dryRun = flagBool(flags, 'dryRun');
  const sheet = maybeSheet(flagStr(flags, 'sheet'));
  ...
  const { ftaUrl } = await resolveUaDutySourceUrls({ ftaUrl: ftaUrlOverride });

  if (!ftaUrl) {
    throw new Error(
      'UA FTA source URL is required. Provide --url=<source> or configure duties.ua.official.fta_excel / UA_FTA_OFFICIAL_EXCEL_URL.'
    );
  }
  ...
  console.log({ step: 'ua-fta-official', ...step });
  const totals: ImportTotals = { count: 0, inserted: 0, updated: 0 };
  addTotals(totals, step);
  return totals;
}

export const dutiesUaMfnOfficial: Command = async (args) => {
  const totals = await runUaMfnOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesUaFtaOfficial: Command = async (args) => {
  const totals = await runUaFtaOfficial(args);
  console.log({ ok: true, ...totals });
};

export const dutiesUaAllOfficial: Command = async (args) => {
  const mfn = await runUaMfnOfficial(args);
  const fta = await runUaFtaOfficial(args);
  console.log({
    ok: true,
