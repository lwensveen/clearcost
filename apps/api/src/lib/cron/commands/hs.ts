// apps/api/src/lib/cron/commands/hs.ts
...
export const hsEuHs6: Command = async () => {
  const payload = await withRun<any>({ source: 'TARIC', job: 'hs:eu-hs6' }, async () => {
    const { importEuHs6FromTaric } = await import(
      '../../../modules/hs-codes/services/aliases/eu/import-cn6-from-taric.js'
    );
