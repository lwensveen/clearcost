  ...
  const payload = await withRun(
    {
      ...
      params: { year, sourceKey: 'hs.wits.sdmx.data_base' },
    },
    async () => {
...
export const hsUsHts10: Command = async () => {
  const payload = await withRun(
    { importSource: 'USITC_HTS', job: 'hs:us-hts10', sourceKey: 'hs.us.usitc.base' },
    async () => {
...
export const hsUk10: Command = async () => {
  const payload = await withRun(
    { importSource: 'UK_TT', job: 'hs:uk10', sourceKey: 'hs.uk.tariff.api_base' },
    async () => {
...
export const hsAhtn: Command = async (args) => {
  ...
  const payload = await withRun(
    { importSource: 'AHTN', job: 'hs:ahtn', sourceKey: 'hs.asean.ahtn.csv', params: { url } },
    async () => {
...
export const hsEuHs6: Command = async () => {
  const payload = await withRun(
    { importSource: 'TARIC', job: 'hs:eu-hs6', sourceKey: 'hs.eu.taric.goods' },
    async () => {
...
export const hsEuTaric: Command = async () => {
  const payload = await withRun(
    { importSource: 'TARIC', job: 'hs:euTaric', sourceKey: 'hs.eu.taric.goods_description' },
    async () => {
