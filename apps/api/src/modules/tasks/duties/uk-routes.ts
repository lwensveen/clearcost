...
import { TasksDutyUkFtaBodySchema, TasksDutyUkMfnBodySchema } from '@clearcost/types';
...
export default function ukDutyRoutes(app: FastifyInstance) {
  ...
  app.post(
    '/cron/import/duties/uk-mfn',
    {
      ...
      schema: { body: TasksDutyUkMfnBodySchema },
      config: {
        importMeta: {
          importSource: 'UK_TT',
  ...
  app.post(
    ...
    {
      preHandler: app.requireApiKey(['tasks:duties:uk']),
      schema: { body: TasksDutyUkFtaBodySchema },
      config: {
        importMeta: {
          importSource: 'UK_TT',
    ...
    async (req, reply) => {
      const { hs6, partners, batchSize, apiBaseUrl } = TasksDutyUkFtaBodySchema.parse(
        req.body ?? {}
      );
      const importId = req.importCtx?.runId;
