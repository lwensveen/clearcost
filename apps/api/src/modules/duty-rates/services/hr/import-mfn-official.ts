...
export type ImportHrMfnOfficialOptions = {
...
export type ImportHrMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('croatia')) score += 4;
  if (token.includes('croatian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`HR MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'hr-mfn-source';
  ...
  if (!selected) {
    throw new Error('HR MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importHrMfnOfficial(
  options: ImportHrMfnOfficialOptions
): Promise<ImportHrMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect HR MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Croatia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `hr:mfn:${row.hs6}`,
  });
