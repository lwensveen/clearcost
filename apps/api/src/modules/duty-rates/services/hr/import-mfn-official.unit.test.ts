...
import { importHrMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-hr-mfn-official-'));
  const path = join(dir, 'croatia-tariff.xlsx');
...
describe('importHrMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importHrMfnOfficial({
      urlOrPath: path,
      importId: 'imp_hr_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('hr:mfn:850440');
  });
});
