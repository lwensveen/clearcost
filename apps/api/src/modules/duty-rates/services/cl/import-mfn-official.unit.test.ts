...
import { importClMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-cl-mfn-official-'));
  const path = join(dir, 'chile-tariff.xlsx');
...
describe('importClMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 CLP/kg' },
    ...
    const out = await importClMfnOfficial({
      urlOrPath: path,
      importId: 'imp_cl_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('cl:mfn:850440');
  });
});
