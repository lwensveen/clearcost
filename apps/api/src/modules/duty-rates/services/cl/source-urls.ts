...
export const CL_MFN_OFFICIAL_SOURCE_KEY = 'duties.cl.official.mfn_excel';
export const CL_FTA_OFFICIAL_SOURCE_KEY = 'duties.cl.official.fta_excel';

export type ResolveClDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveClDutySourceUrls(opts: ResolveClDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.CL_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    CL_FTA_OFFICIAL_SOURCE_KEY,
    process.env.CL_FTA_OFFICIAL_EXCEL_URL
  );
