...
export type ImportClMfnOfficialOptions = {
...
export type ImportClMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('chile')) score += 4;
  if (token.includes('chilean')) score += 4;
  if (token.includes('aduana')) score += 3;
  if (token.includes('arancel')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`CL MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'cl-mfn-source';
  ...
  if (!selected) {
    throw new Error('CL MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importClMfnOfficial(
  options: ImportClMfnOfficialOptions
): Promise<ImportClMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect CL MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Chile customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `cl:mfn:${row.hs6}`,
  });
