...
export const NL_MFN_OFFICIAL_SOURCE_KEY = 'duties.nl.official.mfn_excel';
export const NL_FTA_OFFICIAL_SOURCE_KEY = 'duties.nl.official.fta_excel';

export type ResolveNlDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveNlDutySourceUrls(opts: ResolveNlDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.NL_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    NL_FTA_OFFICIAL_SOURCE_KEY,
    process.env.NL_FTA_OFFICIAL_EXCEL_URL
  );
