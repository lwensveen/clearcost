...
export type ImportNlMfnOfficialOptions = {
...
export type ImportNlMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('netherlands')) score += 4;
  if (token.includes('dutch')) score += 4;
  if (token.includes('douane')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`NL MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'nl-mfn-source';
  ...
  if (!selected) {
    throw new Error('NL MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importNlMfnOfficial(
  options: ImportNlMfnOfficialOptions
): Promise<ImportNlMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect NL MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Netherlands customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `nl:mfn:${row.hs6}`,
  });
