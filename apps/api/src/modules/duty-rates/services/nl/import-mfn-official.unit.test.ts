...
import { importNlMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-nl-mfn-official-'));
  const path = join(dir, 'netherlands-tariff.xlsx');
...
describe('importNlMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importNlMfnOfficial({
      urlOrPath: path,
      importId: 'imp_nl_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('nl:mfn:850440');
  });
});
