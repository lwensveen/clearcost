...
export type ImportGhMfnOfficialOptions = {
...
export type ImportGhMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('ghana')) score += 4;
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'gh-mfn-source';
  ...
  if (!selected) {
    throw new Error('GH MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importGhMfnOfficial(
  options: ImportGhMfnOfficialOptions
): Promise<ImportGhMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect GH MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Ghana customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `gh:mfn:${row.hs6}`,
  });
