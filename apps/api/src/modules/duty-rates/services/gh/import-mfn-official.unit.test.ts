...
import { importGhMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-gh-mfn-official-'));
  const path = join(dir, 'ghana-tariff.xlsx');
...
describe('importGhMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importGhMfnOfficial({
      urlOrPath: path,
      importId: 'imp_gh_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('gh:mfn:850440');
  });
});
