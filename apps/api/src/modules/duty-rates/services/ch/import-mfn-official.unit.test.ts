...
import { importChMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ch-mfn-official-'));
  const path = join(dir, 'swiss-tariff.xlsx');
...
describe('importChMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      { 'Tariff Item': '85044010', 'MFN Rate': '3.5%' },
      { 'Tariff Item': '85183000', 'MFN Rate': 'Free' },
      { 'Tariff Item': '85291000', 'MFN Rate': '12 CHF/100kg' },
    ...
    const out = await importChMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ch_mfn',
    });
    ...
    expect(rows).toEqual([
      makeRow({ hs6: '850440', ratePct: '3.5' }),
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ch:mfn:850440');
  });
});
