...
export type ImportChMfnOfficialOptions = {
...
export type ImportChMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('swiss')) score += 4;
  if (token.includes('tares')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`CH MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ch-mfn-source';
  ...
  if (!selected) {
    throw new Error('CH MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importChMfnOfficial(
  options: ImportChMfnOfficialOptions
): Promise<ImportChMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect CH MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Swiss customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ch:mfn:${row.hs6}`,
  });
