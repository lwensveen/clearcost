...
import { importAiMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ai-mfn-official-'));
  const path = join(dir, 'ai-tariff.xlsx');
...
describe('importAiMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAiMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ai_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ai:mfn:850440');
  });
});
