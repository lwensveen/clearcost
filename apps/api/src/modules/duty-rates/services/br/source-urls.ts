...
export const BR_MFN_OFFICIAL_SOURCE_KEY = 'duties.br.official.mfn_excel';
export const BR_FTA_OFFICIAL_SOURCE_KEY = 'duties.br.official.fta_excel';

export type ResolveBrDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveBrDutySourceUrls(opts: ResolveBrDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.BR_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    BR_FTA_OFFICIAL_SOURCE_KEY,
    process.env.BR_FTA_OFFICIAL_EXCEL_URL
  );
