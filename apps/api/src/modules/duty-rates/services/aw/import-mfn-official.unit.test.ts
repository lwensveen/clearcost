...
import { importAwMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-aw-mfn-official-'));
  const path = join(dir, 'aw-tariff.xlsx');
...
describe('importAwMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAwMfnOfficial({
      urlOrPath: path,
      importId: 'imp_aw_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('aw:mfn:850440');
  });
});
