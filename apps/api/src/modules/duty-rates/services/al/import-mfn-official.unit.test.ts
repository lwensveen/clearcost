...
import { importAlMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-al-mfn-official-'));
  const path = join(dir, 'al-tariff.xlsx');
...
describe('importAlMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAlMfnOfficial({
      urlOrPath: path,
      importId: 'imp_al_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('al:mfn:850440');
  });
});
