...
export const AL_MFN_OFFICIAL_SOURCE_KEY = 'duties.al.official.mfn_excel';
export const AL_FTA_OFFICIAL_SOURCE_KEY = 'duties.al.official.fta_excel';

export type ResolveAlDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveAlDutySourceUrls(opts: ResolveAlDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.AL_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    AL_FTA_OFFICIAL_SOURCE_KEY,
    process.env.AL_FTA_OFFICIAL_EXCEL_URL
  );
