...
export type ImportAlMfnOfficialOptions = {
...
export type ImportAlMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('albania')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error('AL MFN official source download failed ' + response.status);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'al-mfn-source';
  ...
  if (!selected) {
    throw new Error('AL MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAlMfnOfficial(
  options: ImportAlMfnOfficialOptions
): Promise<ImportAlMfnOfficialSummary> {
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Albania customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `al:mfn:${row.hs6}`,
  });
