...
export type ImportCoMfnOfficialOptions = {
...
export type ImportCoMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('colombia')) score += 4;
  if (token.includes('colombian')) score += 4;
  if (token.includes('tariff')) score += 4;
  if (token.includes('customs')) score += 3;
  if (token.includes('dian')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`CO MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'co-mfn-source';
  ...
  if (!selected) {
    throw new Error('CO MFN source ZIP did not contain a readable workbook entry');
  }
  const workbookBuffer = await selected.buffer();
  return { workbookBuffer, sourceFile: selected.path };
}

export async function importCoMfnOfficial(
  options: ImportCoMfnOfficialOptions
): Promise<ImportCoMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect CO MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Colombia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `co:mfn:${row.hs6}`,
  });
