...
export const HU_MFN_OFFICIAL_SOURCE_KEY = 'duties.hu.official.mfn_excel';
export const HU_FTA_OFFICIAL_SOURCE_KEY = 'duties.hu.official.fta_excel';

export type ResolveHuDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveHuDutySourceUrls(opts: ResolveHuDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.HU_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    HU_FTA_OFFICIAL_SOURCE_KEY,
    process.env.HU_FTA_OFFICIAL_EXCEL_URL
  );
