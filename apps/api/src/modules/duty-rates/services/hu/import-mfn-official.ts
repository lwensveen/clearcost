...
export type ImportHuMfnOfficialOptions = {
...
export type ImportHuMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('hungary')) score += 4;
  if (token.includes('hungarian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`HU MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'hu-mfn-source';
  ...
  if (!selected) {
    throw new Error('HU MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importHuMfnOfficial(
  options: ImportHuMfnOfficialOptions
): Promise<ImportHuMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect HU MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Hungary customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `hu:mfn:${row.hs6}`,
  });
