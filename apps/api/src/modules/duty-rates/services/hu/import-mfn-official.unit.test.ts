...
import { importHuMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-hu-mfn-official-'));
  const path = join(dir, 'hungary-tariff.xlsx');
...
describe('importHuMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importHuMfnOfficial({
      urlOrPath: path,
      importId: 'imp_hu_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('hu:mfn:850440');
  });
});
