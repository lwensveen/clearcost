...
export type ImportPtMfnOfficialOptions = {
...
export type ImportPtMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('portugal')) score += 4;
  if (token.includes('portuguese')) score += 4;
  if (token.includes('alfandega')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`PT MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'pt-mfn-source';
  ...
  if (!selected) {
    throw new Error('PT MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importPtMfnOfficial(
  options: ImportPtMfnOfficialOptions
): Promise<ImportPtMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect PT MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Portugal customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `pt:mfn:${row.hs6}`,
  });
