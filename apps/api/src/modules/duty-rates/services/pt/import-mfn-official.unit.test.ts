...
import { importPtMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-pt-mfn-official-'));
  const path = join(dir, 'portugal-tariff.xlsx');
...
describe('importPtMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 PLN/kg' },
    ...
    const out = await importPtMfnOfficial({
      urlOrPath: path,
      importId: 'imp_pt_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('pt:mfn:850440');
  });
});
