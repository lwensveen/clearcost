...
import { importUaMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ua-mfn-official-'));
  const path = join(dir, 'ukraine-tariff.xlsx');
...
describe('importUaMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importUaMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ua_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ua:mfn:850440');
  });
});
