...
export type ImportUaMfnOfficialOptions = {
...
export type ImportUaMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('ukraine')) score += 4;
  if (token.includes('ukrainian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`UA MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ua-mfn-source';
  ...
  if (!selected) {
    throw new Error('UA MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importUaMfnOfficial(
  options: ImportUaMfnOfficialOptions
): Promise<ImportUaMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect UA MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Ukraine customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ua:mfn:${row.hs6}`,
  });
