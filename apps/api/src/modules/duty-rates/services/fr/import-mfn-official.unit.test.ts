...
import { importFrMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-fr-mfn-official-'));
  const path = join(dir, 'france-tariff.xlsx');
...
describe('importFrMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importFrMfnOfficial({
      urlOrPath: path,
      importId: 'imp_fr_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('fr:mfn:850440');
  });
});
