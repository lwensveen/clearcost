...
export type ImportFrMfnOfficialOptions = {
...
export type ImportFrMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('france')) score += 4;
  if (token.includes('french')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`FR MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'fr-mfn-source';
  ...
  if (!selected) {
    throw new Error('FR MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importFrMfnOfficial(
  options: ImportFrMfnOfficialOptions
): Promise<ImportFrMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect FR MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'France customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `fr:mfn:${row.hs6}`,
  });
