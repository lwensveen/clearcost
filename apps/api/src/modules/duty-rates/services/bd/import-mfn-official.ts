...
export type ImportBdMfnOfficialOptions = {
...
export type ImportBdMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('bangladesh')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`BD MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'bd-mfn-source';
  ...
  if (!selected) {
    throw new Error('BD MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importBdMfnOfficial(
  options: ImportBdMfnOfficialOptions
): Promise<ImportBdMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect BD MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Bangladesh customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `bd:mfn:${row.hs6}`,
  });
