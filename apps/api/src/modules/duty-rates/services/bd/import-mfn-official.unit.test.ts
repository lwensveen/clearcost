...
import { importBdMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-bd-mfn-official-'));
  const path = join(dir, 'bangladesh-tariff.xlsx');
...
describe('importBdMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importBdMfnOfficial({
      urlOrPath: path,
      importId: 'imp_bd_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('bd:mfn:850440');
  });
});
