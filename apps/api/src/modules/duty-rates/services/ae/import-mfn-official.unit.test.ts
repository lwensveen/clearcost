...
import { importAeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ae-mfn-official-'));
  const path = join(dir, 'uae-tariff.xlsx');
...
describe('importAeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ae_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ae:mfn:850440');
  });
});
