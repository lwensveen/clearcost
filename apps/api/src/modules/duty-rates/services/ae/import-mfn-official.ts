...
export type ImportAeMfnOfficialOptions = {
...
export type ImportAeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('uae')) score += 4;
  if (token.includes('emirates')) score += 4;
  if (token.includes('emirati')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`AE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ae-mfn-source';
  ...
  if (!selected) {
    throw new Error('AE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAeMfnOfficial(
  options: ImportAeMfnOfficialOptions
): Promise<ImportAeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect AE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'United Arab Emirates customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ae:mfn:${row.hs6}`,
  });
