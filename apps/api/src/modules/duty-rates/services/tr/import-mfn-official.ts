...
export type ImportTrMfnOfficialOptions = {
...
export type ImportTrMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('turkey')) score += 4;
  if (token.includes('turkiye')) score += 4;
  if (token.includes('turkish')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  ...
  if (isHttpLike(safePath)) {
    ...
    if (!response.ok) {
      throw new Error(`TR MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'tr-mfn-source';
  ...
  if (!selected) {
    throw new Error('TR MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importTrMfnOfficial(
  options: ImportTrMfnOfficialOptions
): Promise<ImportTrMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect TR MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `tr:mfn:${row.hs6}`,
  });
