...
export const TR_MFN_OFFICIAL_SOURCE_KEY = 'duties.tr.official.mfn_excel';
export const TR_FTA_OFFICIAL_SOURCE_KEY = 'duties.tr.official.fta_excel';

export type ResolveTrDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveTrDutySourceUrls(opts: ResolveTrDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.TR_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    TR_FTA_OFFICIAL_SOURCE_KEY,
    process.env.TR_FTA_OFFICIAL_EXCEL_URL
  );
