...
export type ImportGrMfnOfficialOptions = {
...
export type ImportGrMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('greece')) score += 4;
  if (token.includes('greek')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`GR MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'gr-mfn-source';
  ...
  if (!selected) {
    throw new Error('GR MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importGrMfnOfficial(
  options: ImportGrMfnOfficialOptions
): Promise<ImportGrMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect GR MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Greece customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `gr:mfn:${row.hs6}`,
  });
