...
import { importGrMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-gr-mfn-official-'));
  const path = join(dir, 'greece-tariff.xlsx');
...
describe('importGrMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importGrMfnOfficial({
      urlOrPath: path,
      importId: 'imp_gr_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('gr:mfn:850440');
  });
});
