...
export const TW_MFN_OFFICIAL_SOURCE_KEY = 'duties.tw.official.mfn_excel';
export const TW_FTA_OFFICIAL_SOURCE_KEY = 'duties.tw.official.fta_excel';

export type ResolveTwDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveTwDutySourceUrls(opts: ResolveTwDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.TW_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    TW_FTA_OFFICIAL_SOURCE_KEY,
    process.env.TW_FTA_OFFICIAL_EXCEL_URL
  );
