...
export type ImportTwMfnOfficialOptions = {
...
export type ImportTwMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('taiwan')) score += 4;
  if (token.includes('customs')) score += 4;
  if (token.includes('tariff')) score += 4;
  if (token.includes('mof')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`TW MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'tw-mfn-source';
  ...
  if (!selected) {
    throw new Error('TW MFN source ZIP did not contain a readable workbook entry');
  }
  const workbookBuffer = await selected.buffer();
  return { workbookBuffer, sourceFile: selected.path };
}

export async function importTwMfnOfficial(
  options: ImportTwMfnOfficialOptions
): Promise<ImportTwMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect TW MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Taiwan customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `tw:mfn:${row.hs6}`,
  });
