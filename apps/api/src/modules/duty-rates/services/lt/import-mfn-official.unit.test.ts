...
import { importLtMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-lt-mfn-official-'));
  const path = join(dir, 'lithuania-tariff.xlsx');
...
describe('importLtMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importLtMfnOfficial({
      urlOrPath: path,
      importId: 'imp_lt_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('lt:mfn:850440');
  });
});
