...
export type ImportLtMfnOfficialOptions = {
...
export type ImportLtMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('lithuania')) score += 4;
  if (token.includes('lithuanian')) score += 4;
  if (token.includes('muitine')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`LT MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'lt-mfn-source';
  ...
  if (!selected) {
    throw new Error('LT MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importLtMfnOfficial(
  options: ImportLtMfnOfficialOptions
): Promise<ImportLtMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect LT MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Lithuania customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `lt:mfn:${row.hs6}`,
  });
