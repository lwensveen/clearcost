...
export type ImportPkMfnOfficialOptions = {
...
export type ImportPkMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('pakistan')) score += 4;
  if (token.includes('pakistani')) score += 4;
  if (token.includes('fbr')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`PK MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'pk-mfn-source';
  ...
  if (!selected) {
    throw new Error('PK MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importPkMfnOfficial(
  options: ImportPkMfnOfficialOptions
): Promise<ImportPkMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect PK MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Pakistan customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `pk:mfn:${row.hs6}`,
  });
