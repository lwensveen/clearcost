...
import { importPkMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-pk-mfn-official-'));
  const path = join(dir, 'pakistan-tariff.xlsx');
...
describe('importPkMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 PKR/kg' },
    ...
    const out = await importPkMfnOfficial({
      urlOrPath: path,
      importId: 'imp_pk_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('pk:mfn:850440');
  });
});
