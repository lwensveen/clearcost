...
import { importAuMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-au-mfn-official-'));
  const path = join(dir, 'working-tariff.xlsx');
...
describe('importAuMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      { 'Tariff Item': '85044010', General: '5%' },
      { 'Tariff Item': '85183000', General: 'Free' },
      { 'Tariff Item': '85291000', General: '2 AUD/kg' },
      { 'Tariff Item': 'abc', General: '4%' },
    ]);
    ...
    const out = await importAuMfnOfficial({
      urlOrPath: path,
      importId: 'imp_au_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('au:mfn:850440');
  });
});
