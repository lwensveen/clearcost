...
export type ImportAuMfnOfficialOptions = {
...
export type ImportAuMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('workingtariff')) score += 4;
  if (token.includes('working-tariff')) score += 4;
  if (token.includes('tariff')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`AU MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'au-mfn-source';
  ...
  if (candidates.length === 0) {
    // XLSX is a ZIP container; if no nested workbook-like entries are found,
    // parse the source payload directly.
    return {
      workbookBuffer: sourceBuffer,
      sourceFile: sourceName,
    };
...
export async function importAuMfnOfficial(
  options: ImportAuMfnOfficialOptions
): Promise<ImportAuMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect AU MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'AU customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `au:mfn:${row.hs6}`,
  });
