...
import { importEeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ee-mfn-official-'));
  const path = join(dir, 'estonia-tariff.xlsx');
...
describe('importEeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importEeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ee_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ee:mfn:850440');
  });
});
