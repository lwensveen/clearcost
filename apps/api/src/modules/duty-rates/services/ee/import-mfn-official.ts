...
export type ImportEeMfnOfficialOptions = {
...
export type ImportEeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('estonia')) score += 4;
  if (token.includes('estonian')) score += 4;
  if (token.includes('told')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`EE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ee-mfn-source';
  ...
  if (!selected) {
    throw new Error('EE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importEeMfnOfficial(
  options: ImportEeMfnOfficialOptions
): Promise<ImportEeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect EE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Estonia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ee:mfn:${row.hs6}`,
  });
