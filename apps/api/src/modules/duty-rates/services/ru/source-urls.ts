...
export const RU_MFN_OFFICIAL_SOURCE_KEY = 'duties.ru.official.mfn_excel';
export const RU_FTA_OFFICIAL_SOURCE_KEY = 'duties.ru.official.fta_excel';

export type ResolveRuDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveRuDutySourceUrls(opts: ResolveRuDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.RU_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    RU_FTA_OFFICIAL_SOURCE_KEY,
    process.env.RU_FTA_OFFICIAL_EXCEL_URL
  );
