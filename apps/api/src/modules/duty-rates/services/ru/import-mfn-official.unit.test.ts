...
import { importRuMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ru-mfn-official-'));
...
describe('importRuMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Tariff': '3.5 RUB/kg' },
    ...
    const out = await importRuMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ru_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ru:mfn:850440');
  });
});
