...
export type ImportRuMfnOfficialOptions = {
...
export type ImportRuMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('russia')) score += 4;
  if (token.includes('gov.ru')) score += 4;
  if (token.includes('tariff')) score += 4;
  if (token.includes('customs')) score += 3;
  if (token.includes('eaeu')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`RU MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ru-mfn-source';
  ...
  if (!selected) {
    throw new Error('RU MFN source ZIP did not contain a readable workbook entry');
  }
  const workbookBuffer = await selected.buffer();
  return { workbookBuffer, sourceFile: selected.path };
}

export async function importRuMfnOfficial(
  options: ImportRuMfnOfficialOptions
): Promise<ImportRuMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect RU MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Russia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ru:mfn:${row.hs6}`,
  });
