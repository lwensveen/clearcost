...
export const ZA_MFN_OFFICIAL_SOURCE_KEY = 'duties.za.official.mfn_excel';
export const ZA_FTA_OFFICIAL_SOURCE_KEY = 'duties.za.official.fta_excel';

export type ResolveZaDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveZaDutySourceUrls(opts: ResolveZaDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.ZA_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    ZA_FTA_OFFICIAL_SOURCE_KEY,
    process.env.ZA_FTA_OFFICIAL_EXCEL_URL
  );
