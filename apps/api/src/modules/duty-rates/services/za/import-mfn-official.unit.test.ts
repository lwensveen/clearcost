...
import { importZaMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-za-mfn-official-'));
...
describe('importZaMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      { 'Tariff Item': '85044010', 'MFN Tariff': '6.5%' },
      { 'Tariff Item': '85183000', 'MFN Tariff': 'Free' },
      { 'Tariff Item': '85291000', 'MFN Tariff': '3.5 ZAR/kg' },
      { 'Tariff Item': 'abc', 'MFN Tariff': '5%' },
    ]);
    mocks.batchUpsertDutyRatesFromStreamMock.mockResolvedValue({
      ok: true,
      inserted: 2,
    ...
    const out = await importZaMfnOfficial({
      urlOrPath: path,
      importId: 'imp_za_mfn',
    });
    ...
    expect(rows).toEqual([
      makeRow({ hs6: '850440', ratePct: '6.5' }),
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('za:mfn:850440');
  });
});
