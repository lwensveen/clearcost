...
export type ImportZaMfnOfficialOptions = {
...
export type ImportZaMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('southafrica')) score += 4;
  if (token.includes('south-africa')) score += 4;
  if (token.includes('tariff')) score += 4;
  if (token.includes('customs')) score += 3;
  if (token.includes('sars')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`ZA MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'za-mfn-source';
  ...
  if (!selected) {
    throw new Error('ZA MFN source ZIP did not contain a readable workbook entry');
  }
  const workbookBuffer = await selected.buffer();
  return { workbookBuffer, sourceFile: selected.path };
}

export async function importZaMfnOfficial(
  options: ImportZaMfnOfficialOptions
): Promise<ImportZaMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect ZA MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'South Africa customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `za:mfn:${row.hs6}`,
  });
