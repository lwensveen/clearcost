...
import { importRoMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ro-mfn-official-'));
  const path = join(dir, 'romania-tariff.xlsx');
...
describe('importRoMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importRoMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ro_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ro:mfn:850440');
  });
});
