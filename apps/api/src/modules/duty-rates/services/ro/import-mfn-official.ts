...
export type ImportRoMfnOfficialOptions = {
...
export type ImportRoMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('romania')) score += 4;
  if (token.includes('romanian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`RO MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ro-mfn-source';
  ...
  if (!selected) {
    throw new Error('RO MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importRoMfnOfficial(
  options: ImportRoMfnOfficialOptions
): Promise<ImportRoMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect RO MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Romania customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ro:mfn:${row.hs6}`,
  });
