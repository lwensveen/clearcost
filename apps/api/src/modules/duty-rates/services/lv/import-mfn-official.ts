...
export type ImportLvMfnOfficialOptions = {
...
export type ImportLvMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('latvia')) score += 4;
  if (token.includes('latvian')) score += 4;
  if (token.includes('vid')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`LV MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'lv-mfn-source';
  ...
  if (!selected) {
    throw new Error('LV MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importLvMfnOfficial(
  options: ImportLvMfnOfficialOptions
): Promise<ImportLvMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect LV MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Latvia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `lv:mfn:${row.hs6}`,
  });
