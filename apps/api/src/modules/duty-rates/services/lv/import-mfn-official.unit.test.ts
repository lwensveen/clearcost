...
import { importLvMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-lv-mfn-official-'));
  const path = join(dir, 'latvia-tariff.xlsx');
...
describe('importLvMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importLvMfnOfficial({
      urlOrPath: path,
      importId: 'imp_lv_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('lv:mfn:850440');
  });
});
