...
import { importIsMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-is-mfn-official-'));
  const path = join(dir, 'iceland-tariff.xlsx');
...
describe('importIsMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 ISK/kg' },
    ...
    const out = await importIsMfnOfficial({
      urlOrPath: path,
      importId: 'imp_is_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('is:mfn:850440');
  });
});
