...
export type ImportIsMfnOfficialOptions = {
...
export type ImportIsMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('iceland')) score += 4;
  if (token.includes('icelandic')) score += 4;
  if (token.includes('tollskra')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`IS MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'is-mfn-source';
  ...
  if (!selected) {
    throw new Error('IS MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importIsMfnOfficial(
  options: ImportIsMfnOfficialOptions
): Promise<ImportIsMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect IS MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Iceland customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `is:mfn:${row.hs6}`,
  });
