...
export const IS_MFN_OFFICIAL_SOURCE_KEY = 'duties.is.official.mfn_excel';
export const IS_FTA_OFFICIAL_SOURCE_KEY = 'duties.is.official.fta_excel';

export type ResolveIsDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveIsDutySourceUrls(opts: ResolveIsDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.IS_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    IS_FTA_OFFICIAL_SOURCE_KEY,
    process.env.IS_FTA_OFFICIAL_EXCEL_URL
  );
