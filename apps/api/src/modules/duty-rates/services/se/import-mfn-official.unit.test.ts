...
import { importSeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-se-mfn-official-'));
  const path = join(dir, 'sweden-tariff.xlsx');
...
describe('importSeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importSeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_se_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('se:mfn:850440');
  });
});
