...
export type ImportSeMfnOfficialOptions = {
...
export type ImportSeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('sweden')) score += 4;
  if (token.includes('swedish')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`SE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'se-mfn-source';
  ...
  if (!selected) {
    throw new Error('SE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importSeMfnOfficial(
  options: ImportSeMfnOfficialOptions
): Promise<ImportSeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect SE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Sweden customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `se:mfn:${row.hs6}`,
  });
