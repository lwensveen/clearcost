...
export type ImportAdMfnOfficialOptions = {
...
export type ImportAdMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('andorra')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error('AD MFN official source download failed ' + response.status);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ad-mfn-source';
  ...
  if (!selected) {
    throw new Error('AD MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAdMfnOfficial(
  options: ImportAdMfnOfficialOptions
): Promise<ImportAdMfnOfficialSummary> {
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Andorra customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ad:mfn:${row.hs6}`,
  });
