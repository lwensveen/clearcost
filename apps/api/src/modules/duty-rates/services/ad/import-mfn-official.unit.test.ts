...
import { importAdMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ad-mfn-official-'));
  const path = join(dir, 'ad-tariff.xlsx');
...
describe('importAdMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAdMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ad_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ad:mfn:850440');
  });
});
