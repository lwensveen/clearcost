...
export const ES_MFN_OFFICIAL_SOURCE_KEY = 'duties.es.official.mfn_excel';
export const ES_FTA_OFFICIAL_SOURCE_KEY = 'duties.es.official.fta_excel';

export type ResolveEsDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveEsDutySourceUrls(opts: ResolveEsDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.ES_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    ES_FTA_OFFICIAL_SOURCE_KEY,
    process.env.ES_FTA_OFFICIAL_EXCEL_URL
  );
