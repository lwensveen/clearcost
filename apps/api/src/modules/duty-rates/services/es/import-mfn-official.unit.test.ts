...
import { importEsMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-es-mfn-official-'));
  const path = join(dir, 'spain-tariff.xlsx');
...
describe('importEsMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importEsMfnOfficial({
      urlOrPath: path,
      importId: 'imp_es_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('es:mfn:850440');
  });
});
