...
export type ImportEsMfnOfficialOptions = {
...
export type ImportEsMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('spain')) score += 4;
  if (token.includes('spanish')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`ES MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'es-mfn-source';
  ...
  if (!selected) {
    throw new Error('ES MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importEsMfnOfficial(
  options: ImportEsMfnOfficialOptions
): Promise<ImportEsMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect ES MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Spain customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `es:mfn:${row.hs6}`,
  });
