...
import { importAtMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-at-mfn-official-'));
  const path = join(dir, 'austria-tariff.xlsx');
...
describe('importAtMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAtMfnOfficial({
      urlOrPath: path,
      importId: 'imp_at_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('at:mfn:850440');
  });
});
