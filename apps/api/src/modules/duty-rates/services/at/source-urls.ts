...
export const AT_MFN_OFFICIAL_SOURCE_KEY = 'duties.at.official.mfn_excel';
export const AT_FTA_OFFICIAL_SOURCE_KEY = 'duties.at.official.fta_excel';

export type ResolveAtDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveAtDutySourceUrls(opts: ResolveAtDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.AT_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    AT_FTA_OFFICIAL_SOURCE_KEY,
    process.env.AT_FTA_OFFICIAL_EXCEL_URL
  );
