...
export type ImportAtMfnOfficialOptions = {
...
export type ImportAtMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('austria')) score += 4;
  if (token.includes('austrian')) score += 4;
  if (token.includes('zoll')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`AT MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'at-mfn-source';
  ...
  if (!selected) {
    throw new Error('AT MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAtMfnOfficial(
  options: ImportAtMfnOfficialOptions
): Promise<ImportAtMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect AT MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Austria customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `at:mfn:${row.hs6}`,
  });
