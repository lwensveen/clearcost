...
export const LI_MFN_OFFICIAL_SOURCE_KEY = 'duties.li.official.mfn_excel';
export const LI_FTA_OFFICIAL_SOURCE_KEY = 'duties.li.official.fta_excel';

export type ResolveLiDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveLiDutySourceUrls(opts: ResolveLiDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.LI_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    LI_FTA_OFFICIAL_SOURCE_KEY,
    process.env.LI_FTA_OFFICIAL_EXCEL_URL
  );
