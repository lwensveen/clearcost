...
import { importLiMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-li-mfn-official-'));
  const path = join(dir, 'liechtenstein-tariff.xlsx');
...
describe('importLiMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 CHF/kg' },
    ...
    const out = await importLiMfnOfficial({
      urlOrPath: path,
      importId: 'imp_li_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('li:mfn:850440');
  });
});
