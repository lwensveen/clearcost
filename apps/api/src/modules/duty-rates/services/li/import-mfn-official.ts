...
export type ImportLiMfnOfficialOptions = {
...
export type ImportLiMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('liechtenstein')) score += 4;
  if (token.includes('zoll')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`LI MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'li-mfn-source';
  ...
  if (!selected) {
    throw new Error('LI MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importLiMfnOfficial(
  options: ImportLiMfnOfficialOptions
): Promise<ImportLiMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect LI MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Liechtenstein customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `li:mfn:${row.hs6}`,
  });
