...
import { importAgMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ag-mfn-official-'));
  const path = join(dir, 'ag-tariff.xlsx');
...
describe('importAgMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAgMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ag_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ag:mfn:850440');
  });
});
