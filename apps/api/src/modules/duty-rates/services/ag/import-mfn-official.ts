...
export type ImportAgMfnOfficialOptions = {
...
export type ImportAgMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('antigua')) score += 4;
  if (token.includes('barbuda')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error('AG MFN official source download failed ' + response.status);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ag-mfn-source';
  ...
  if (!selected) {
    throw new Error('AG MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAgMfnOfficial(
  options: ImportAgMfnOfficialOptions
): Promise<ImportAgMfnOfficialSummary> {
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Antigua and Barbuda customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ag:mfn:${row.hs6}`,
  });
