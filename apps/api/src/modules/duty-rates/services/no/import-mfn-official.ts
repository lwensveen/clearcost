...
export type ImportNoMfnOfficialOptions = {
...
export type ImportNoMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('norway')) score += 4;
  if (token.includes('norwegian')) score += 4;
  if (token.includes('tolletaten')) score += 3;
  if (token.includes('tolltariff')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`NO MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'no-mfn-source';
  ...
  if (!selected) {
    throw new Error('NO MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importNoMfnOfficial(
  options: ImportNoMfnOfficialOptions
): Promise<ImportNoMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect NO MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Norway customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `no:mfn:${row.hs6}`,
  });
