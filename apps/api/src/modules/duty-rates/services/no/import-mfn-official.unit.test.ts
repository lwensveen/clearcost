...
import { importNoMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-no-mfn-official-'));
  const path = join(dir, 'norway-tariff.xlsx');
...
describe('importNoMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 NOK/kg' },
    ...
    const out = await importNoMfnOfficial({
      urlOrPath: path,
      importId: 'imp_no_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('no:mfn:850440');
  });
});
