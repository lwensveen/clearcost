...
export type ImportBgMfnOfficialOptions = {
...
export type ImportBgMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('bulgaria')) score += 4;
  if (token.includes('bulgarian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`BG MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'bg-mfn-source';
  ...
  if (!selected) {
    throw new Error('BG MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importBgMfnOfficial(
  options: ImportBgMfnOfficialOptions
): Promise<ImportBgMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect BG MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Bulgaria customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `bg:mfn:${row.hs6}`,
  });
