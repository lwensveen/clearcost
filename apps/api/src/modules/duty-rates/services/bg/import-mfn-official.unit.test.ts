...
import { importBgMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-bg-mfn-official-'));
  const path = join(dir, 'bulgaria-tariff.xlsx');
...
describe('importBgMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importBgMfnOfficial({
      urlOrPath: path,
      importId: 'imp_bg_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('bg:mfn:850440');
  });
});
