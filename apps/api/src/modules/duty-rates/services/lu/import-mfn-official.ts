...
export type ImportLuMfnOfficialOptions = {
...
export type ImportLuMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('luxembourg')) score += 4;
  if (token.includes('luxembourgish')) score += 4;
  if (token.includes('douanes')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`LU MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'lu-mfn-source';
  ...
  if (!selected) {
    throw new Error('LU MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importLuMfnOfficial(
  options: ImportLuMfnOfficialOptions
): Promise<ImportLuMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect LU MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Luxembourg customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `lu:mfn:${row.hs6}`,
  });
