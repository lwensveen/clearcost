...
import { importLuMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-lu-mfn-official-'));
  const path = join(dir, 'luxembourg-tariff.xlsx');
...
describe('importLuMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importLuMfnOfficial({
      urlOrPath: path,
      importId: 'imp_lu_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('lu:mfn:850440');
  });
});
