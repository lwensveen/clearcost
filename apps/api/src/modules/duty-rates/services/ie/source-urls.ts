...
export const IE_MFN_OFFICIAL_SOURCE_KEY = 'duties.ie.official.mfn_excel';
export const IE_FTA_OFFICIAL_SOURCE_KEY = 'duties.ie.official.fta_excel';

export type ResolveIeDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveIeDutySourceUrls(opts: ResolveIeDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.IE_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    IE_FTA_OFFICIAL_SOURCE_KEY,
    process.env.IE_FTA_OFFICIAL_EXCEL_URL
  );
