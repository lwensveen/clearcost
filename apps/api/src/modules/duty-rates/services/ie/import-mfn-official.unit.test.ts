...
import { importIeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ie-mfn-official-'));
  const path = join(dir, 'ireland-tariff.xlsx');
...
describe('importIeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importIeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ie_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ie:mfn:850440');
  });
});
