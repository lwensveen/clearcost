...
export type ImportIeMfnOfficialOptions = {
...
export type ImportIeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('ireland')) score += 4;
  if (token.includes('irish')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`IE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ie-mfn-source';
  ...
  if (!selected) {
    throw new Error('IE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importIeMfnOfficial(
  options: ImportIeMfnOfficialOptions
): Promise<ImportIeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect IE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Ireland customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ie:mfn:${row.hs6}`,
  });
