...
export type ImportInMfnOfficialOptions = {
...
export type ImportInMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('india')) score += 4;
  if (token.includes('indian')) score += 4;
  if (token.includes('cbic')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`IN MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'in-mfn-source';
  ...
  if (!selected) {
    throw new Error('IN MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importInMfnOfficial(
  options: ImportInMfnOfficialOptions
): Promise<ImportInMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect IN MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'India customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `in:mfn:${row.hs6}`,
  });
