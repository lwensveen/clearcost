...
import { importInMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-in-mfn-official-'));
  const path = join(dir, 'india-tariff.xlsx');
...
describe('importInMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importInMfnOfficial({
      urlOrPath: path,
      importId: 'imp_in_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('in:mfn:850440');
  });
});
