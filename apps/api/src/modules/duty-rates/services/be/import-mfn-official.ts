...
export type ImportBeMfnOfficialOptions = {
...
export type ImportBeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('belgium')) score += 4;
  if (token.includes('belgian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`BE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'be-mfn-source';
  ...
  if (!selected) {
    throw new Error('BE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importBeMfnOfficial(
  options: ImportBeMfnOfficialOptions
): Promise<ImportBeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect BE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Belgium customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `be:mfn:${row.hs6}`,
  });
