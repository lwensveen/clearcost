...
import { importBeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-be-mfn-official-'));
  const path = join(dir, 'belgium-tariff.xlsx');
...
describe('importBeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importBeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_be_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('be:mfn:850440');
  });
});
