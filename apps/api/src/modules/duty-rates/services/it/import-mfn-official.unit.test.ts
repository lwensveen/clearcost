...
import { importItMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-it-mfn-official-'));
  const path = join(dir, 'italy-tariff.xlsx');
...
describe('importItMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importItMfnOfficial({
      urlOrPath: path,
      importId: 'imp_it_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('it:mfn:850440');
  });
});
