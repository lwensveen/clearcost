...
export type ImportItMfnOfficialOptions = {
...
export type ImportItMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('italy')) score += 4;
  if (token.includes('italian')) score += 4;
  if (token.includes('dogane')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`IT MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'it-mfn-source';
  ...
  if (!selected) {
    throw new Error('IT MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importItMfnOfficial(
  options: ImportItMfnOfficialOptions
): Promise<ImportItMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect IT MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Italy customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `it:mfn:${row.hs6}`,
  });
