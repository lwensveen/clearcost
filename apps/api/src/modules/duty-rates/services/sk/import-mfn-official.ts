...
export type ImportSkMfnOfficialOptions = {
...
export type ImportSkMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('slovakia')) score += 4;
  if (token.includes('slovak')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`SK MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'sk-mfn-source';
  ...
  if (!selected) {
    throw new Error('SK MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importSkMfnOfficial(
  options: ImportSkMfnOfficialOptions
): Promise<ImportSkMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect SK MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Slovakia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `sk:mfn:${row.hs6}`,
  });
