...
import { importSkMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-sk-mfn-official-'));
  const path = join(dir, 'slovakia-tariff.xlsx');
...
describe('importSkMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importSkMfnOfficial({
      urlOrPath: path,
      importId: 'imp_sk_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('sk:mfn:850440');
  });
});
