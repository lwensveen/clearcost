...
import { importNgMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ng-mfn-official-'));
  const path = join(dir, 'nigeria-tariff.xlsx');
...
describe('importNgMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importNgMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ng_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ng:mfn:850440');
  });
});
