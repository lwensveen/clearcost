...
export const NG_MFN_OFFICIAL_SOURCE_KEY = 'duties.ng.official.mfn_excel';
export const NG_FTA_OFFICIAL_SOURCE_KEY = 'duties.ng.official.fta_excel';

export type ResolveNgDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveNgDutySourceUrls(opts: ResolveNgDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.NG_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    NG_FTA_OFFICIAL_SOURCE_KEY,
    process.env.NG_FTA_OFFICIAL_EXCEL_URL
  );
