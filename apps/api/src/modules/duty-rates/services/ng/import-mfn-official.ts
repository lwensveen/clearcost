...
export type ImportNgMfnOfficialOptions = {
...
export type ImportNgMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('nigeria')) score += 4;
  if (token.includes('nigerian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`NG MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ng-mfn-source';
  ...
  if (!selected) {
    throw new Error('NG MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importNgMfnOfficial(
  options: ImportNgMfnOfficialOptions
): Promise<ImportNgMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect NG MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Nigeria customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ng:mfn:${row.hs6}`,
  });
