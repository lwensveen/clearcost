...
export const IL_MFN_OFFICIAL_SOURCE_KEY = 'duties.il.official.mfn_excel';
export const IL_FTA_OFFICIAL_SOURCE_KEY = 'duties.il.official.fta_excel';

export type ResolveIlDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveIlDutySourceUrls(opts: ResolveIlDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.IL_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    IL_FTA_OFFICIAL_SOURCE_KEY,
    process.env.IL_FTA_OFFICIAL_EXCEL_URL
  );
