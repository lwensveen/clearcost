...
import { importAmMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-am-mfn-official-'));
  const path = join(dir, 'am-tariff.xlsx');
...
describe('importAmMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAmMfnOfficial({
      urlOrPath: path,
      importId: 'imp_am_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('am:mfn:850440');
  });
});
