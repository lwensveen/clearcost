...
import { importCyMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-cy-mfn-official-'));
  const path = join(dir, 'cyprus-tariff.xlsx');
...
describe('importCyMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importCyMfnOfficial({
      urlOrPath: path,
      importId: 'imp_cy_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('cy:mfn:850440');
  });
});
