...
export type ImportCyMfnOfficialOptions = {
...
export type ImportCyMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('cyprus')) score += 4;
  if (token.includes('cypriot')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`CY MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'cy-mfn-source';
  ...
  if (!selected) {
    throw new Error('CY MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importCyMfnOfficial(
  options: ImportCyMfnOfficialOptions
): Promise<ImportCyMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect CY MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Cyprus customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `cy:mfn:${row.hs6}`,
  });
