...
import { importMtMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-mt-mfn-official-'));
  const path = join(dir, 'malta-tariff.xlsx');
...
describe('importMtMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importMtMfnOfficial({
      urlOrPath: path,
      importId: 'imp_mt_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('mt:mfn:850440');
  });
});
