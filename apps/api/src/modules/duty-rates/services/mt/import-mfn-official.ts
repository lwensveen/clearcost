...
export type ImportMtMfnOfficialOptions = {
...
export type ImportMtMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('malta')) score += 4;
  if (token.includes('maltese')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`MT MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'mt-mfn-source';
  ...
  if (!selected) {
    throw new Error('MT MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importMtMfnOfficial(
  options: ImportMtMfnOfficialOptions
): Promise<ImportMtMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect MT MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Malta customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `mt:mfn:${row.hs6}`,
  });
