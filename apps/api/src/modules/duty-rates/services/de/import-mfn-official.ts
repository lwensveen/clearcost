...
export type ImportDeMfnOfficialOptions = {
...
export type ImportDeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('germany')) score += 4;
  if (token.includes('german')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`DE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'de-mfn-source';
  ...
  if (!selected) {
    throw new Error('DE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importDeMfnOfficial(
  options: ImportDeMfnOfficialOptions
): Promise<ImportDeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect DE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Germany customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `de:mfn:${row.hs6}`,
  });
