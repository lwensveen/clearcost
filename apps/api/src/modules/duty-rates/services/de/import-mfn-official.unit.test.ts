...
import { importDeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-de-mfn-official-'));
  const path = join(dir, 'germany-tariff.xlsx');
...
describe('importDeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importDeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_de_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('de:mfn:850440');
  });
});
