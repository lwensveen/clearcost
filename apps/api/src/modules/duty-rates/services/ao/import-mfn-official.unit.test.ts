...
import { importAoMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ao-mfn-official-'));
  const path = join(dir, 'ao-tariff.xlsx');
...
describe('importAoMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAoMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ao_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ao:mfn:850440');
  });
});
