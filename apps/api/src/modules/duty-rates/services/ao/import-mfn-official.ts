...
export type ImportAoMfnOfficialOptions = {
...
export type ImportAoMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('angola')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error('AO MFN official source download failed ' + response.status);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ao-mfn-source';
  ...
  if (!selected) {
    throw new Error('AO MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAoMfnOfficial(
  options: ImportAoMfnOfficialOptions
): Promise<ImportAoMfnOfficialSummary> {
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Angola customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ao:mfn:${row.hs6}`,
  });
