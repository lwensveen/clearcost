...
import { importAfMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-af-mfn-official-'));
  const path = join(dir, 'af-tariff.xlsx');
...
describe('importAfMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAfMfnOfficial({
      urlOrPath: path,
      importId: 'imp_af_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('af:mfn:850440');
  });
});
