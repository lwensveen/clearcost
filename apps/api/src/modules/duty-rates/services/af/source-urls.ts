...
export const AF_MFN_OFFICIAL_SOURCE_KEY = 'duties.af.official.mfn_excel';
export const AF_FTA_OFFICIAL_SOURCE_KEY = 'duties.af.official.fta_excel';

export type ResolveAfDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveAfDutySourceUrls(opts: ResolveAfDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.AF_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    AF_FTA_OFFICIAL_SOURCE_KEY,
    process.env.AF_FTA_OFFICIAL_EXCEL_URL
  );
