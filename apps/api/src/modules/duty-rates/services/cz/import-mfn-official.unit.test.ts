...
import { importCzMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-cz-mfn-official-'));
  const path = join(dir, 'czechia-tariff.xlsx');
...
describe('importCzMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importCzMfnOfficial({
      urlOrPath: path,
      importId: 'imp_cz_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('cz:mfn:850440');
  });
});
