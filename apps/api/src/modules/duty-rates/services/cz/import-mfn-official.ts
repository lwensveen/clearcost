...
export type ImportCzMfnOfficialOptions = {
...
export type ImportCzMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('czechia')) score += 4;
  if (token.includes('czech')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`CZ MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'cz-mfn-source';
  ...
  if (!selected) {
    throw new Error('CZ MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importCzMfnOfficial(
  options: ImportCzMfnOfficialOptions
): Promise<ImportCzMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect CZ MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Czechia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `cz:mfn:${row.hs6}`,
  });
