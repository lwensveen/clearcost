...
export type ImportMxMfnOfficialOptions = {
...
export type ImportMxMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('tigie')) score += 4;
  if (token.includes('mexico')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`MX MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'mx-mfn-source';
  ...
  if (!selected) {
    throw new Error('MX MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importMxMfnOfficial(
  options: ImportMxMfnOfficialOptions
): Promise<ImportMxMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect MX MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Mexico customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `mx:mfn:${row.hs6}`,
  });
