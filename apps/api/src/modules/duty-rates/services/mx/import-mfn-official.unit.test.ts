...
import { importMxMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-mx-mfn-official-'));
  const path = join(dir, 'tigie.xlsx');
...
describe('importMxMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 USD/kg' },
    ...
    const out = await importMxMfnOfficial({
      urlOrPath: path,
      importId: 'imp_mx_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('mx:mfn:850440');
  });
});
