...
export type ImportEgMfnOfficialOptions = {
...
export type ImportEgMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('egypt')) score += 4;
  if (token.includes('egyptian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`EG MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'eg-mfn-source';
  ...
  if (!selected) {
    throw new Error('EG MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importEgMfnOfficial(
  options: ImportEgMfnOfficialOptions
): Promise<ImportEgMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect EG MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Egypt customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `eg:mfn:${row.hs6}`,
  });
