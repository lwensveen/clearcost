...
import { importEgMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-eg-mfn-official-'));
  const path = join(dir, 'egypt-tariff.xlsx');
...
describe('importEgMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importEgMfnOfficial({
      urlOrPath: path,
      importId: 'imp_eg_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('eg:mfn:850440');
  });
});
