...
export type ImportSiMfnOfficialOptions = {
...
export type ImportSiMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('slovenia')) score += 4;
  if (token.includes('slovenian')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`SI MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'si-mfn-source';
  ...
  if (!selected) {
    throw new Error('SI MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importSiMfnOfficial(
  options: ImportSiMfnOfficialOptions
): Promise<ImportSiMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect SI MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Slovenia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `si:mfn:${row.hs6}`,
  });
