...
export const SI_MFN_OFFICIAL_SOURCE_KEY = 'duties.si.official.mfn_excel';
export const SI_FTA_OFFICIAL_SOURCE_KEY = 'duties.si.official.fta_excel';

export type ResolveSiDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveSiDutySourceUrls(opts: ResolveSiDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.SI_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    SI_FTA_OFFICIAL_SOURCE_KEY,
    process.env.SI_FTA_OFFICIAL_EXCEL_URL
  );
