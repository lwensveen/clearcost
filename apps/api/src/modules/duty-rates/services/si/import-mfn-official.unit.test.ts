...
import { importSiMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-si-mfn-official-'));
  const path = join(dir, 'slovenia-tariff.xlsx');
...
describe('importSiMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importSiMfnOfficial({
      urlOrPath: path,
      importId: 'imp_si_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('si:mfn:850440');
  });
});
