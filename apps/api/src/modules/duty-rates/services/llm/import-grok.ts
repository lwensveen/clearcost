...
const pct = (v?: number | null) =>
  typeof v === 'number' && Number.isFinite(v) ? (Math.round(v * 1000) / 1000).toFixed(3) : '0.000';

const LLM_SOURCE: DutyInsert['source'] =
  (process.env.LLM_DUTY_SOURCE as DutyInsert['source']) ?? 'llm';
...
export async function importDutyRatesFromGrok(
  ...
  const rows: DutyInsert[] = payload.rows.map((row) => ({
    ...
    hs6: row.hs6,
    dutyRule: row.duty_rule,
    ratePct: pct(row.components.find((c) => c.type === 'advalorem')?.rate_pct ?? null),
