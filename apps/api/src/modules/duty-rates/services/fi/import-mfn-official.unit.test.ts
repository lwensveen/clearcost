...
import { importFiMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-fi-mfn-official-'));
  const path = join(dir, 'finland-tariff.xlsx');
...
describe('importFiMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importFiMfnOfficial({
      urlOrPath: path,
      importId: 'imp_fi_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('fi:mfn:850440');
  });
});
