...
export type ImportFiMfnOfficialOptions = {
...
export type ImportFiMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('finland')) score += 4;
  if (token.includes('finnish')) score += 4;
  if (token.includes('tulli')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`FI MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'fi-mfn-source';
  ...
  if (!selected) {
    throw new Error('FI MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importFiMfnOfficial(
  options: ImportFiMfnOfficialOptions
): Promise<ImportFiMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect FI MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Finland customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `fi:mfn:${row.hs6}`,
  });
