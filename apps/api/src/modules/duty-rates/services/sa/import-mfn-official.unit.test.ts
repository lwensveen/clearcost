...
import { importSaMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-sa-mfn-official-'));
  const path = join(dir, 'saudi-tariff.xlsx');
...
describe('importSaMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importSaMfnOfficial({
      urlOrPath: path,
      importId: 'imp_sa_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('sa:mfn:850440');
  });
});
