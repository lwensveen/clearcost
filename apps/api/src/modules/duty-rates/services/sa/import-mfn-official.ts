...
export type ImportSaMfnOfficialOptions = {
...
export type ImportSaMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('saudi')) score += 4;
  if (token.includes('ksa')) score += 4;
  if (token.includes('saudiarabia')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`SA MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'sa-mfn-source';
  ...
  if (!selected) {
    throw new Error('SA MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importSaMfnOfficial(
  options: ImportSaMfnOfficialOptions
): Promise<ImportSaMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect SA MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Saudi Arabia customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `sa:mfn:${row.hs6}`,
  });
