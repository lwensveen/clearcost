...
import { importPlMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-pl-mfn-official-'));
  const path = join(dir, 'poland-tariff.xlsx');
...
describe('importPlMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importPlMfnOfficial({
      urlOrPath: path,
      importId: 'imp_pl_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('pl:mfn:850440');
  });
});
