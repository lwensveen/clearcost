...
export const PL_MFN_OFFICIAL_SOURCE_KEY = 'duties.pl.official.mfn_excel';
export const PL_FTA_OFFICIAL_SOURCE_KEY = 'duties.pl.official.fta_excel';

export type ResolvePlDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolvePlDutySourceUrls(opts: ResolvePlDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.PL_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    PL_FTA_OFFICIAL_SOURCE_KEY,
    process.env.PL_FTA_OFFICIAL_EXCEL_URL
  );
