...
export type ImportPlMfnOfficialOptions = {
...
export type ImportPlMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('poland')) score += 4;
  if (token.includes('polish')) score += 4;
  if (token.includes('minfin')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`PL MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'pl-mfn-source';
  ...
  if (!selected) {
    throw new Error('PL MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importPlMfnOfficial(
  options: ImportPlMfnOfficialOptions
): Promise<ImportPlMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect PL MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Poland customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `pl:mfn:${row.hs6}`,
  });
