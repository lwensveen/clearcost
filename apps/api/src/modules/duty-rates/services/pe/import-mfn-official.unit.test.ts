...
import { importPeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-pe-mfn-official-'));
  const path = join(dir, 'peru-tariff.xlsx');
...
describe('importPeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 PEN/kg' },
    ...
    const out = await importPeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_pe_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('pe:mfn:850440');
  });
});
