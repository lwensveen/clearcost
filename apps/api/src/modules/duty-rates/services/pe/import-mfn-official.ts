...
export type ImportPeMfnOfficialOptions = {
...
export type ImportPeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('peru')) score += 4;
  if (token.includes('peruvian')) score += 4;
  if (token.includes('sunat')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`PE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'pe-mfn-source';
  ...
  if (!selected) {
    throw new Error('PE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importPeMfnOfficial(
  options: ImportPeMfnOfficialOptions
): Promise<ImportPeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect PE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Peru customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `pe:mfn:${row.hs6}`,
  });
