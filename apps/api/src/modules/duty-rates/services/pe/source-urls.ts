...
export const PE_MFN_OFFICIAL_SOURCE_KEY = 'duties.pe.official.mfn_excel';
export const PE_FTA_OFFICIAL_SOURCE_KEY = 'duties.pe.official.fta_excel';

export type ResolvePeDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolvePeDutySourceUrls(opts: ResolvePeDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.PE_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    PE_FTA_OFFICIAL_SOURCE_KEY,
    process.env.PE_FTA_OFFICIAL_EXCEL_URL
  );
