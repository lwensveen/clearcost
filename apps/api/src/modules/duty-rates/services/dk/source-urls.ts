...
export const DK_MFN_OFFICIAL_SOURCE_KEY = 'duties.dk.official.mfn_excel';
export const DK_FTA_OFFICIAL_SOURCE_KEY = 'duties.dk.official.fta_excel';

export type ResolveDkDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveDkDutySourceUrls(opts: ResolveDkDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.DK_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    DK_FTA_OFFICIAL_SOURCE_KEY,
    process.env.DK_FTA_OFFICIAL_EXCEL_URL
  );
