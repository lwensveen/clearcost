...
import { importDkMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-dk-mfn-official-'));
  const path = join(dir, 'denmark-tariff.xlsx');
...
describe('importDkMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importDkMfnOfficial({
      urlOrPath: path,
      importId: 'imp_dk_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('dk:mfn:850440');
  });
});
