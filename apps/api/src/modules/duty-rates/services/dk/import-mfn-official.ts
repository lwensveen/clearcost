...
export type ImportDkMfnOfficialOptions = {
...
export type ImportDkMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('denmark')) score += 4;
  if (token.includes('danish')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`DK MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'dk-mfn-source';
  ...
  if (!selected) {
    throw new Error('DK MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importDkMfnOfficial(
  options: ImportDkMfnOfficialOptions
): Promise<ImportDkMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect DK MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Denmark customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `dk:mfn:${row.hs6}`,
  });
