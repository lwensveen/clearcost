...
export type ImportArMfnOfficialOptions = {
...
export type ImportArMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('argentina')) score += 4;
  if (token.includes('argentina.gob')) score += 4;
  if (token.includes('tariff')) score += 4;
  if (token.includes('customs')) score += 3;
  if (token.includes('ncm')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`AR MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ar-mfn-source';
  ...
  if (!selected) {
    throw new Error('AR MFN source ZIP did not contain a readable workbook entry');
  }
  const workbookBuffer = await selected.buffer();
  return { workbookBuffer, sourceFile: selected.path };
}

export async function importArMfnOfficial(
  options: ImportArMfnOfficialOptions
): Promise<ImportArMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect AR MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Argentina customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ar:mfn:${row.hs6}`,
  });
