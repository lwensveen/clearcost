...
import { importPaMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-pa-mfn-official-'));
  const path = join(dir, 'panama-tariff.xlsx');
...
describe('importPaMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    const path = await writeFixtureXlsx([
      ...
      { 'Tariff Item': '85291000', 'MFN Rate': '10 PAB/kg' },
    ...
    const out = await importPaMfnOfficial({
      urlOrPath: path,
      importId: 'imp_pa_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('pa:mfn:850440');
  });
});
