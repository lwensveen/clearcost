...
export type ImportPaMfnOfficialOptions = {
...
export type ImportPaMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('panama')) score += 4;
  if (token.includes('panamanian')) score += 4;
  if (token.includes('aduanas')) score += 3;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`PA MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'pa-mfn-source';
  ...
  if (!selected) {
    throw new Error('PA MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importPaMfnOfficial(
  options: ImportPaMfnOfficialOptions
): Promise<ImportPaMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect PA MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Panama customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `pa:mfn:${row.hs6}`,
  });
