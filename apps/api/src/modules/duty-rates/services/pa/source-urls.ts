...
export const PA_MFN_OFFICIAL_SOURCE_KEY = 'duties.pa.official.mfn_excel';
export const PA_FTA_OFFICIAL_SOURCE_KEY = 'duties.pa.official.fta_excel';

export type ResolvePaDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolvePaDutySourceUrls(opts: ResolvePaDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.PA_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    PA_FTA_OFFICIAL_SOURCE_KEY,
    process.env.PA_FTA_OFFICIAL_EXCEL_URL
  );
