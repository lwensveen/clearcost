...
import { importKeMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-ke-mfn-official-'));
  const path = join(dir, 'kenya-tariff.xlsx');
...
describe('importKeMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importKeMfnOfficial({
      urlOrPath: path,
      importId: 'imp_ke_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('ke:mfn:850440');
  });
});
