...
export const KE_MFN_OFFICIAL_SOURCE_KEY = 'duties.ke.official.mfn_excel';
export const KE_FTA_OFFICIAL_SOURCE_KEY = 'duties.ke.official.fta_excel';

export type ResolveKeDutySourceUrlsOptions = {
  mfnUrl?: string;
  ftaUrl?: string;
};
...
export async function resolveKeDutySourceUrls(opts: ResolveKeDutySourceUrlsOptions = {}): Promise<{
  mfnUrl?: string;
  ftaUrl?: string;
}> {
  const mfnUrl = await resolveOptionalSourceUrl(
    ...
    process.env.KE_MFN_OFFICIAL_EXCEL_URL
  );
  const ftaUrl = await resolveOptionalSourceUrl(
    opts.ftaUrl,
    KE_FTA_OFFICIAL_SOURCE_KEY,
    process.env.KE_FTA_OFFICIAL_EXCEL_URL
  );
