...
export type ImportKeMfnOfficialOptions = {
...
export type ImportKeMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('kenya')) score += 4;
  if (token.includes('kenyan')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error(`KE MFN official source download failed ${response.status}`);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'ke-mfn-source';
  ...
  if (!selected) {
    throw new Error('KE MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importKeMfnOfficial(
  options: ImportKeMfnOfficialOptions
): Promise<ImportKeMfnOfficialSummary> {
  ...
  if (!columns) {
    throw new Error(`Unable to detect KE MFN columns. Headers: ${headers.join(', ')}`);
  }
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'Kenya customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `ke:mfn:${row.hs6}`,
  });
