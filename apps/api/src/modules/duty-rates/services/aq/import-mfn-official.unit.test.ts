...
import { importAqMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-aq-mfn-official-'));
  const path = join(dir, 'aq-tariff.xlsx');
...
describe('importAqMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAqMfnOfficial({
      urlOrPath: path,
      importId: 'imp_aq_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('aq:mfn:850440');
  });
});
