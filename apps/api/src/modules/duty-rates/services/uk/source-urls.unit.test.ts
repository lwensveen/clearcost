...
describe('resolveUkTariffDutySourceUrls', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  it('uses explicit override without querying source registry', async () => {
    const out = await resolveUkTariffDutySourceUrls({ apiBaseUrl: 'https://override.test' });
  ...
  it('resolves UK tariff API base from source registry with env fallback', async () => {
    ...
    const out = await resolveUkTariffDutySourceUrls();
