...
export type ImportAsMfnOfficialOptions = {
...
export type ImportAsMfnOfficialSummary = {
  ok: true;
  inserted: number;
  updated: number;
  count: number;
...
function scoreArchiveEntry(path: string): number {
  ...
  let score = 0;
  if (token.includes('american')) score += 4;
  if (token.includes('samoa')) score += 4;
...
async function loadSourceBuffer(urlOrPath: string): Promise<Buffer> {
  if (isHttpLike(urlOrPath)) {
    ...
    if (!response.ok) {
      throw new Error('AS MFN official source download failed ' + response.status);
    }
...
async function resolveWorkbookBuffer(urlOrPath: string): Promise<ResolvedWorkbook> {
  ...
  const sourceName = basename(urlOrPath) || 'as-mfn-source';
  ...
  if (!selected) {
    throw new Error('AS MFN source ZIP did not contain a readable workbook entry');
  }
...
export async function importAsMfnOfficial(
  options: ImportAsMfnOfficialOptions
): Promise<ImportAsMfnOfficialSummary> {
  ...
  for (const row of rawRows) {
    ...
    rowsToUpsert.push({
      ...
      ratePct,
      currency: undefined,
      notes: 'American Samoa customs tariff MFN (official)',
    });
    kept += 1;
  }
  ...
  const result = await batchUpsertDutyRatesFromStream(rowsToUpsert, {
    ...
    source: 'official',
    makeSourceRef: (row) => `as:mfn:${row.hs6}`,
  });
