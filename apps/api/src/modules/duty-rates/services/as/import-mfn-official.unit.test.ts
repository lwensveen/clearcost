...
import { importAsMfnOfficial } from './import-mfn-official.js';
...
async function writeFixtureXlsx(rows: Array<Record<string, unknown>>, sheetName = 'Tariff') {
  const dir = await mkdtemp(join(tmpdir(), 'clearcost-as-mfn-official-'));
  const path = join(dir, 'as-tariff.xlsx');
...
describe('importAsMfnOfficial', () => {
  ...
  it('parses HS6 + ad-valorem MFN rates and skips non-ad-valorem rows', async () => {
    ...
    const out = await importAsMfnOfficial({
      urlOrPath: path,
      importId: 'imp_as_mfn',
    });
    ...
    expect(opts.makeSourceRef(makeRow({ hs6: '850440' }))).toBe('as:mfn:850440');
  });
});
